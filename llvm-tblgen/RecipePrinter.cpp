...
namespace llvm {
template <> struct GraphTraits<AllSteps *> {
...
private:
  ...
  inline void printSimple(raw_ostream &OS, StringRef FieldName,
                          Record *SimpleRecord) {
    OS << SimpleRecord->getValueAsString(FieldName);
  }
  inline void printUnit(raw_ostream &OS, Record *UnitRecord) {
    printSimple(OS, "Text", UnitRecord);
  }
  inline void printEquipment(raw_ostream &OS, Record *EquipRecord) {
    printSimple(OS, "Name", EquipRecord);
  }

  void printFixedPoint(raw_ostream &OS, Record *FPRecord);

  void printNumeric(raw_ostream &OS, Record *NumericRecord, StringRef ValField,
                    StringRef UnitField);

  inline void printTemperature(raw_ostream &OS, Record *TempRecord) {
    printNumeric(OS, TempRecord, "Value", "TempUnit");
  }

  inline void printDuration(raw_ostream &OS, Record *DurationRecord) {
  ...
  /// functions
  inline void printRecord(raw_ostream &OS, Record *R) {
...
void RecipePrinter::printIngredient(raw_ostream &OS, Record *IngredientRecord,
                                    bool WithQuantity) {
...
void RecipePrinter::printAction(raw_ostream &OS, Record *ActionRecord) {
  printEquipment(OS << "use ", ActionRecord->getValueAsDef("Using"));
  auto Text = ActionRecord->getValueAsString("Text");
  OS << " to " << Text;
}

void RecipePrinter::printStepReference(raw_ostream &OS, Record *StepRecord) {
  assert(StepIndicies.count(StepRecord));
  OS << "outcome from (step " << StepIndicies[StepRecord] + 1 << ")";
}

void RecipePrinter::printCustomStep(raw_ostream &OS, Record *StepRecord) {
